...
const EMPTY_FORM: Omit<CmRecap, "id" | "created_at"> = {
  title: "",
  event_date: null,
  venue: null,
  recap: null,
...
export default function RecapsAdminPage() {
  const [records, setRecords] = useState<CmRecap[]>([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [editing, setEditing] = useState<CmRecap | null>(null)
  ...
  async function load() {
    ...
    if (data) setRecords(data as CmRecap[])
    setLoading(false)
  }
  ...
  function openEdit(row: Record<string, unknown>) {
    ...
    setForm({
      ...
      featured_photo_url: r.featured_photo_url,
      tags: r.tags,
      published: r.published,
      sort_order: r.sort_order,
    })
    setTagsInput(r.tags ? r.tags.join(", ") : "")
    setError(null)
    setShowForm(true)
  }
  ...
  async function handleSave() {
    ...
    const payload = { ...form, tags, event_date: form.event_date || null }
    const supabase = createClient()
    let err

    if (editing) {
      const { error: e } = await supabase.from("cm_recaps").update(payload).eq("id", editing.id)
      err = e
    } else {
      const { error: e } = await supabase.from("cm_recaps").insert(payload)
      err = e
    }
  ...
  async function handleDelete(id: string) {
    const supabase = createClient()
    await supabase.from("cm_recaps").delete().eq("id", id)
    await load()
  }

  async function handleTogglePublish(id: string, current: boolean) {
    const supabase = createClient()
    await supabase.from("cm_recaps").update({ published: !current }).eq("id", id)
    await load()
  }
  ...
  const columns = [
    ...
    { header: "Attendees", key: "attendee_count", render: (v: unknown) => v ? String(v) : "—" },
    ...
    <div className="p-8 max-w-5xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="font-bold uppercase" style={{ fontFamily: "var(--font-display)", fontSize: "24px", color: "#f0ead8" }}>Event Recaps</h1>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(6,12,26,0.9)" }}>
          <div className="w-full max-w-xl rounded-2xl border p-7 overflow-y-auto" style={{ backgroundColor: "#0d1a35", borderColor: "rgba(201,168,76,0.2)", maxHeight: "90vh" }}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-bold uppercase text-sm" style={{ fontFamily: "var(--font-display)", color: "#c9a84c" }}>{editing ? "Edit Recap" : "New Recap"}</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[9px] font-semibold uppercase tracking-[0.18em] mb-1.5" style={{ color: "rgba(240,234,216,0.4)" }}>Attendee Count</label>
                  <input type="number" value={form.attendee_count ?? ""} onChange={(e) => setForm({ ...form, attendee_count: e.target.value ? parseInt(e.target.value) : null })} className="admin-input" />
                </div>
              </div>
              <div>
                <label className="block text-[9px] font-semibold uppercase tracking-[0.18em] mb-1.5" style={{ color: "rgba(240,234,216,0.4)" }}>Venue</label>
                <input value={form.venue ?? ""} onChange={(e) => setForm({ ...form, venue: e.target.value || null })} className="admin-input" />
              </div>
              <div>
                <label className="block text-[9px] font-semibold uppercase tracking-[0.18em] mb-1.5" style={{ color: "rgba(240,234,216,0.4)" }}>Featured Photo URL</label>
                <input type="url" value={form.featured_photo_url ?? ""} onChange={(e) => setForm({ ...form, featured_photo_url: e.target.value || null })} className="admin-input" />
              </div>
              <div>
                <label className="block text-[9px] font-semibold uppercase tracking-[0.18em] mb-1.5" style={{ color: "rgba(240,234,216,0.4)" }}>Recap</label>
                <textarea rows={4} value={form.recap ?? ""} onChange={(e) => setForm({ ...form, recap: e.target.value || null })} className="admin-input resize-none" />
              </div>
              <div>
                <label className="block text-[9px] font-semibold uppercase tracking-[0.18em] mb-1.5" style={{ color: "rgba(240,234,216,0.4)" }}>Tags (comma-separated)</label>
                <input
                  value={tagsInput}
                  onChange={(e) => setTagsInput(e.target.value)}
                  placeholder="Bitcoin, AI, Web3"
